...
详细安装步骤可以参考IK分词器的安装过程。[《安装elasticsearch.md#3安装ik分词器》](https://github.com/qingdog/cloud/blob/master/day05-Elasticsearch01/资料/安装elasticsearch.md#3安装ik分词器)
...
> 发送 POST 请求到 Elasticsearch，路径为 **/_analyze**，并在请求正文中指定要分析的文本和要使用的分析器。**/_analyze** 是 Elasticsearch 内置的 API，用于分析文本，可以用于验证分析器的配置是否正确、测试分析器的效果等。
...
参考课前资料的文档：[《安装elasticsearch.md#4部署es集群》](https://github.com/qingdog/cloud/blob/master/day05-Elasticsearch01/资料/安装elasticsearch.md#4部署es集群)
